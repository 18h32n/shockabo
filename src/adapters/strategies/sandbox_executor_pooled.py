...
class SandboxExecutor:
    ...
    def _initialize_pool(self):
        ...
        try:
            ...
            for _i in range(min(2, self.config.process_pool.pool_size)):
                self._create_worker()
    ...
    def _create_worker(self) -> str | None:
        ...
        with self._worker_lock:
            ...
            try:
                ...
                while (worker.state == WorkerState.STARTING and
    ...
    def _handle_execution_request(self, request_data: tuple):
        ...
        try:
            ...
            # Wait for response
            try:
                response = worker.response_queue.get(timeout=request.timeout + 1.0)
                ...
                # Worker didn't respond in time, remove it and fallback
                self._remove_worker(worker_id)
                result = self._execute_in_new_process(request)
                future.set_result(result)
    ...
    def _worker_main_loop(self, worker_id: str, request_queue: Queue, response_queue: Queue):
        """Main loop for worker processes."""
        try:
            # Set resource limits
            self._set_resource_limits()

            # Signal that we're ready by setting state to idle
            # (This is done implicitly when the worker starts listening)
            ...
            while True:
                try:
                    ...
                    # Execute the request
                    response = self._execute_request_in_worker(request)
    ...
    def _execute_request_in_worker(self, request: ExecutionRequest) -> dict[str, Any]:
        """Execute a request within a worker process."""
        try:
            # Create restricted execution environment
            restricted_globals = self._create_restricted_globals_for_request(request)
            ...
            if request.function_name not in restricted_globals:
                return {
                    ...
                    'error_context': None,
                    'metrics': self._create_error_metrics(time.time() - start_time),
                    'timed_out': False,
                    'memory_exceeded': False
                }
            ...
            return {
                ...
                'metrics': self._create_error_metrics(time.time() - start_time if 'start_time' in locals() else 0),
                'timed_out': False,
                'memory_exceeded': False
            }
    ...
    def _clear_worker_state(self):
        ...
        # Note: We can't clear everything as it would break the worker,
        # but we rely on the restricted globals to prevent cross-contamination

    def _create_restricted_globals_for_request(self, request: ExecutionRequest) -> dict[str, Any]:
    ...
    def _create_error_metrics(self, execution_time: float) -> ExecutionMetrics:
    ...
    def _execute_with_pool(self, code: str, function_name: str, grid: list[list[int]]) -> ExecutionResult:
        ...
        # Wait for result
        try:
            return future.result(timeout=request.timeout + 2.0)
        except Exception:
    ...
    def _worker_process(self, code: str, function_name: str, grid: list[list[int]],
        ...
        """Worker process that executes the code (original single-process version)."""
        try:
            # Set resource limits
            self._set_resource_limits()
