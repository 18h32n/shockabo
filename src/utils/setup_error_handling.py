...
from .error_recovery import get_health_monitor, get_circuit_breaker, CircuitBreakerConfig
...
def setup_error_handling_system(
    ...
    try:
        ...
        if enable_health_monitoring:
            ...
            health_monitor.add_health_check("application", check_app_health, critical=True)
            health_monitor.add_health_check("memory", check_memory_health, critical=False)
        ...
        if enable_circuit_breakers:
            ...
            logger.info("circuit_breakers_initialized", count=len(setup_results["circuit_breakers_initialized"]))
...
def create_fastapi_app_with_error_handling() -> FastAPI:
    ...
    async def health_check():
        ...
        from .error_recovery import get_all_health_statuses, get_all_circuit_breaker_stats
        
        health_statuses = get_all_health_statuses()
        circuit_breaker_stats = get_all_circuit_breaker_stats()
        
        overall_healthy = all(
            status.get("overall_healthy", True) 
